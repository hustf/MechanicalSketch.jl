...
"Source position"
p_source = (3.0, 0.0)m |> ComplexQuantity
"Flow rate, 2d flow"
q_source = 1.0m²/s
"Vortex position"
p_vortex = (0.0, 1.0)m |> ComplexQuantity
"Vorticity, 2d flow"
K = 1.0m²/s / 2π
...
"""
∇ϕ(p) = generate_∇fz_Z_to_Z_from_fz(ϕ, typeof(1.0m), typeof(1.0m²∙s⁻¹))(p)
...
B = quantities_at_pixels(clamped_∇ϕ,
    physwidth = physwidth,
    height_relative_width = height_relative_width);
